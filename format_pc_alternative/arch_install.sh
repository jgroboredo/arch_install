...
if lspci | grep -q -i "VGA.*\WIntel"; then
    ARCH_GPU_TYPE="$ARCH_GPU_TYPE intel"
fi
if lspci | grep -q -i "3D.*\WNVIDIA"; then
    ARCH_GPU_TYPE="$ARCH_GPU_TYPE nvidia"
    elif lspci | grep -q -i "VGA.*\WNVIDIA"; then
    ARCH_GPU_TYPE="$ARCH_GPU_TYPE nvidia"
fi
if lspci | grep -q -i "VGA.*\WAMD"; then
    ARCH_GPU_TYPE="$ARCH_GPU_TYPE amd"
fi
if lspci | grep -q -i "VGA.*\WATI"; then
    ARCH_GPU_TYPE="$ARCH_GPU_TYPE ati"
fi
if [ "$ARCH_VM" != 'none' ]; then
    ARCH_GPU_TYPE="$ARCH_GPU_TYPE $ARCH_VM"
fi
if [[ "$ARCH_GPU_TYPE" != *"nvidia"* ]] && [[ "$ARCH_GPU_TYPE" != *"intel"* ]]; then
    ARCH_GPU_TYPE="$ARCH_GPU_TYPE optimus"
    elif [[ "$ARCH_GPU_TYPE" != *"nvidia"* ]] && [[ "$ARCH_GPU_TYPE" != *"amd"* ]]; then
    ARCH_GPU_TYPE="$ARCH_GPU_TYPE optimus"
fi
if [ -z "$ARCH_GPU_TYPE" ]; then
    lspci | grep 'VGA\|3D'
...
function dot_files() {
    ...
    chmod -R 755 "$ROOT_DIR/files/usr"
    # deploy dotfiles
